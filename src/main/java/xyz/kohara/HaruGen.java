            ...
            recipeDataMap.forEach((name, recipeData) -> {
              try {
                ...
                for (String item : ingredients.keySet()) {
                  writer
                      .append("   if entity @e[type=item,distance=..1.5,tag=stellarity.altar_of_the_accursed." + item +
                          ",scores={stellarity.altar_of_the_accursed.count=" + ingredients.get(item) + "}] \\\n");
                }
                switch (recipeData.resultType()) {
                  case LOOT -> writer.append(
                      "   run function stellarity:mechanic/altar_of_the_accursed/crafting/macro/craft_generic {loot:\""
                          + recipeData.result() + "\"}\n");
                  case LOOT_PARENT -> writer.append(
                      "   run function stellarity:mechanic/altar_of_the_accursed/crafting/macro/craft_weapon {loot:\""
                          + recipeData.result() + "\", parent:\"" + recipeData.parent() + "\"}\n");
